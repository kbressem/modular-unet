# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/01_unet.ipynb (unless otherwise specified).
...
from .utils import create_body, Hook, hook_outputs, model_sizes, dummy_eval, _get_sz_change_idxs
...
class DynamicUnet3D(nn.Module):
    ...
    def __init__(self, encoder, n_out, img_size, act_cls=nn.ReLU, norm_type='Batch',
        ...
        for i,idx in enumerate(sz_chg_idxs):
            ...
            x = unet_block(x)
            if self.deep_supervision: ds.append(DeepSupervision(x.shape[1], n_out, img_size))
        ...
        if img_size != sizes[0][-3:]:
            ...
            if self.deep_supervision: ds.append(DeepSupervision(input_channels, n_out, img_size))

        self.layers = nn.ModuleList(layers)
        if self.deep_supervision: self.ds = nn.ModuleList(ds)

        # Construct the final layer
        self.final = ConvLayer(n_out * len(ds) if self.deep_supervision else input_channels, n_out, kernel_size=1, stride=1, act_cls=None, norm_type=None,  **kwargs)
